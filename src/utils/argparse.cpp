...
static void throw_invalid_cmd_args() { throw std::invalid_argument("unknown command argument"); }

static void throw_invalid_parser() { throw std::invalid_argument("invalid parser"); }
...
void ArgumentParser::add_keyword_arg(const std::string& short_name, const std::string& long_name,
                                     const std::string& action, bool required, const std::string& _default,
                                     const std::string& help)
{
    m_keyword_parsers.emplace_back(short_name, long_name, action, required, _default, help);
    ...
    if (short_name != "") {
        if (m_existing_names.find(short_name.substr(1)) != m_existing_names.end()) {
            printf("duplicate argument: %s\n", short_name.c_str());
            throw_invalid_parser();
        } else {
...
void ArgumentParser::add_positional_arg(const std::string& name, const std::string& narg, const std::string& help)
{
...
ArgumentResult ArgumentParser::parse_args(int argc, char** argv)
{
    ...
    for (auto& p : m_keyword_parsers) p.start_parsing(result);
    for (auto& p : m_positional_parsers) p.start_parsing(result);
    ...
    for (auto& p : m_keyword_parsers) p.end_parsing();
    for (auto& p : m_positional_parsers) p.end_parsing();
...
void ArgumentParser::print_help() const
{
    ...
    for (auto& p : m_positional_parsers) printf(" %s", p.brief_usage().c_str());
    for (auto& p : m_keyword_parsers) printf(" %s", p.brief_usage().c_str());
    printf("\n\nArguments:\n");
    for (auto& p : m_positional_parsers) p.print_help();
    for (auto& p : m_keyword_parsers) p.print_help();
}

ArgumentParser::KeywordParser::KeywordParser(const std::string& short_name, const std::string& long_name,
                                             const std::string& action, bool required, const std::string& _default,
                                             const std::string& help)
    ...
    , m_help(help)
{
    ...
    if (m_action != "" && m_action != "store_true" && m_action != "store_false") {
...
{
    if (m_default != "")
        printf("  %s\n    %s (default: %s)\n\n", brief_usage().c_str(), m_help.c_str(), m_default.c_str());
    else
...
std::string ArgumentParser::KeywordParser::brief_usage() const
{
    ...
    if (m_short_name != "") ret += m_short_name + "/";
    ret += m_long_name;

    if (m_action == "") ret += " <" + m_arg_name + ">";

    if (!m_required) ret += "]";
    return ret;
}

ArgumentParser::PositionalParser::PositionalParser(const std::string& name, const std::string& narg,
                                                   const std::string& help)
    : m_name(name)
    , m_narg(narg)
    , m_help(help)
...
bool ArgumentParser::PositionalParser::consume(std::list<const char*>& args)
{
    ...
    if (m_narg == "1") {
        ...
        if (m_result_buffer->multiargs.find(m_name) == m_result_buffer->multiargs.end())
